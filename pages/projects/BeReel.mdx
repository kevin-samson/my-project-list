...
A Flask-based webtool that gives you a customized timelapse of your favourite BeReal memories.
...
## Backgroud
By the end of 2022, BeReal combined all the photos taken in a year and made a timelape. However, for some reason the timelapce was stopped in 2023.
Searching across the internet, I came accross this project made by [theOneAndOnlyOne](https://github.com/theOneAndOnlyOne). It did not give the output I wanted, so I decided to decided to folk the project and make my own version of it.

## Generating the 1st two seconds
The first two second is all the photos taken put into 2 second. This is done by using the `opencv` and `moviepy` library. Also, a text is overlayed on top of the image.
